...
module.exports = async function (params, srv) {
    ...
    this.on("createOrder", async (req) => {
        try {
            ...
            let ProcessedReq = JSON.parse(req.data.amount);
            if (ProcessedReq.orderId != '') {
                // const { orderId } = ProcessedReq.orderId; // Order ID sent from frontend

                // Fetch order details from Razorpay
                const order = await razorpay.orders.fetch(ProcessedReq.orderId);

                return order;
            } else {
                const { amount } = ProcessedReq.amount; // Amount sent from frontend

                const options = {
                    amount: ProcessedReq.amount * 100,
                    currency: "INR",
            ...
            req.error(500, "Failed to create order");
        }
    ...
    this.on('quotationFun', async (req) => {
        ...
        debugger
        if (VehicleData && VehicleData.length > 0 && req.data.value === 'check') {
            debugger
            if(LoadingStatus[0].companyCode && LoadingStatus[0].purchGroup){
              
            }
            else{
                debugger
                 
                  return 'empty enquiry fields';
            }
            VehicleData.forEach(vehicle => {
                debugger
                if (vehicle.plant === null) {
                    flag = 1;
                }
            });
            if (flag === 1) {
                
                return 'empty Plant Data';
            }
        }
    ...
    this.before('UPDATE', PurchaseOrder, async (req) => {
        if (req.data.status === 'Credit Approved') {
            ...
            const discount1 = (parseFloat(purchase.grandTotal) - parseFloat(purchase.taxAmount) ) - parseFloat(purchase.totalPrice);
            ...
            const paymentDetails = {
                ...
                grandTotal: {
                    subtotal: parseFloat(purchase.totalPrice) || 59000.00,
    ...
    this.after('READ', EnquiryVehicle.drafts, (req) => {
        for (const veh of req) {
            if (veh.discount) {
                ...
                console.log(veh.discount, veh.isChecked);
            } else {
